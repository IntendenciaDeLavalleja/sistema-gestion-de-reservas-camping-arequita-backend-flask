...
def camping_hero_images():
    if request.method == 'POST':
        ...
        try:
            ...
            db.session.rollback()
            if object_name:
                minio_service.remove_object(object_name)
            flash('No se pudo subir la imagen de portada', 'error')
            return redirect(url_for('admin.camping_hero_images'))
...
def toggle_hero_image(image_id):
    try:
        ...
        db.session.rollback()
        flash('No se pudo actualizar el estado de la imagen', 'error')
    return redirect(url_for('admin.camping_hero_images'))
...
def delete_hero_image(image_id):
    try:
        ...
        db.session.rollback()
        flash('No se pudo eliminar la imagen de portada', 'error')
    return redirect(url_for('admin.camping_hero_images'))
