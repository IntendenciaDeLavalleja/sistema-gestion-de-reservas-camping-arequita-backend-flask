    ...
    try:
        ...
        for reservation in expired:
            ...
            reservation.archive_reason = (
                'Expiración automática de 48 horas sin confirmación'
            )

        # 2. Auto-complete finished stays (active -> completed)
        # Check if check_out date has passed.
        # At end of day usually, but let's use current date.
        ...
        for res in finished:
            ...
            if service:
                service.available_units = min(
                    service.total_units,
                    service.available_units + 1,
                )
        ...
        db.session.rollback()
        # Nunca romper request por una tarea de mantenimiento.
        return 0


def confirm_pre_reservation(
    pre_reservation: PreReservation,
) -> tuple[bool, str]:
    if pre_reservation.status != 'pendiente':
        return (
            False,
            f'La pre-reserva ya está en estado {pre_reservation.status}',
        )
    ...
    if pre_reservation.expires_at <= datetime.utcnow():
        ...
        return (
            False,
            'La pre-reserva expiró y fue movida a Expiradas automáticamente',
        )

    service = (
        CampingService.query
        .filter_by(id=pre_reservation.service_id)
        .with_for_update()
        .first()
    )
    if not service:
